...
void test_richardson_extrapolation() noexcept
{
    std::cout << "|---- test_richardson_extrapolation()" << std::endl;
    ...
    using RKM = Classic_fourth_order_method<T_arg_u, T_arg_t>;
    using RE = Richardson_extrapolation<T_arg_u, T_arg_t>;

    constexpr T_arg_t min_frequency = 0.01;
    constexpr T_arg_t max_frequency = 10.0;
    ...
    constexpr T_arg_t max_error = 1.0e-10;
    ...
    std::unique_ptr<ODE_solver> solver =
        std::make_unique<RE>(std::make_unique<RKM>(), accuracy_order_boost);

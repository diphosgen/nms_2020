...
void test_wave_equation_2D_2() noexcept
{
    std::cout << "      |---- test_wave_equation_2D_2()" << std::endl;

    using namespace Math_structures;

    constexpr int max_harmonic_x = 1;
    constexpr int max_harmonic_y = 1;
    ...
    while (t <= time_of_compute) {
        ...
        if (time_step % time_steps_between_checks == 0) {
            const auto exact_solution = grid_exact_solution(t);
